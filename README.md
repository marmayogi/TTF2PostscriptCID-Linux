...
- In order to build this utility in Linux, the two source files were copied to  Linux platform (Ubuntu) and `dos2unix` was invoked to replace CR-LF with LF. A make file has been added that builds the binary. A separate README.md was written and added in the folder. All four files have been checked in to GitHub.
